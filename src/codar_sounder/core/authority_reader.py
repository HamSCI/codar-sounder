...
Under the RTP-reference labeling invariant, codar-sounder labels each
CPI's start time from the RTP sample counter plus this published
offset.  Wire-compatible with the psk-recorder / wspr-recorder
siblings (same JSON schema).
...
the CPI label lands on a useful UTC bin (CODAR CPIs are tens of
seconds long, so accuracy needed is tens of seconds for ionogram
